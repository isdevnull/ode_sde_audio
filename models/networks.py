    ...
    def __init__(
        ...
        for level, mult in reversed(list(enumerate(channel_mult))):
            ...
            if level == len(channel_mult) - 1:
                ...
                output_padding = 1 if level == 0 else 0
                self.dec[f"{res}x{res}_up"] = UNetBlock(
                    in_channels=cout, out_channels=cout, up=True, output_padding=output_padding, **block_kwargs
                )
            for idx in range(num_blocks + 1):
                cin = cout + skips.pop()
                cout = model_channels * mult
...
class STFTUnet(torch.nn.Module):
    def __init__(
        self,
        img_resolution,  # Image resolution.
        img_channels,  # Number of color channels.
    ...
    def internal_forward(self, x, sigma, **model_kwargs):
        x = x.to(torch.float32)
        sigma = sigma.to(torch.float32).reshape(-1, 1, 1, 1)
        #dtype = torch.float32
    ...
    def forward(self, x, sigma, **model_kwargs):
        if not self.spectral:
            x = x.squeeze(1)
            x = torch.stft(x, n_fft=512, win_length=512, hop_length=128, window=self.window, normalized=True, return_complex=True, center=True)
            x = torch.view_as_real(x) # B, F, T, 2
            x = x.permute(0, 3, 1, 2) # B, 2, F, T

        x_hat = self.internal_forward(x, sigma, **model_kwargs)

        if not self.spectral:
            x_hat = torch.view_as_complex(x_hat.permute(0, 2, 3, 1).contiguous())
            x_hat = torch.istft(x_hat, n_fft=512, win_length=512, hop_length=128, window=self.window, normalized=True, center=True)
