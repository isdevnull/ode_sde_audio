...
def triangle_func(
    t: torch.tensor, a: float = 1e-9, b: float = 1.3e-4, *args, **kwargs
) -> torch.tensor:
    """
...
def constant_func(
    t: torch.Tensor, epsilon: float = 1e-4, *args, **kwargs
) -> torch.tensor:
    ...
    **net_conds,
    ...
    def drift(t, x):
        ...
        if adaptive_band:
            ...
            mask = torch.arange(b_shape[-1], device=t.device).unsqueeze(0).expand(
                b_shape[0], -1
            ) < cutoff_indices.unsqueeze(1)
            band[mask] = 1
            net_conds["band"] = band
...
class Diffusion:
    def __init__(
        ...
        self.beta_func = get_beta_function(
            beta_type, a=beta_min, b=beta_max, epsilon=beta_max
        )
    ...
    def get_variance(self, t: torch.Tensor):
        ...
        if self.beta_type == "triangle":
            ...
            sigma1[t <= 0.5] = (t[t <= 0.5] ** 2) * (
                self.beta_max - self.beta_min
            ) + self.beta_min * t[t <= 0.5]
            sigma1[t > 0.5] = (self.beta_max - self.beta_min) * (
                2 * t[t > 0.5] - t[t > 0.5] ** 2 - 0.75
            )
            sigma2[t <= 0.5] = self.beta_min * (1 - t[t <= 0.5]) + (
                1 - t[t <= 0.5] ** 2
            ) * (self.beta_max - self.beta_min)
    ...
    def generate(
        self, net, x0, n_steps: int = 100, return_trajectory: bool = False, **net_conds
    ):
        if self.spectral:
            x0 = torch.stft(
                x0.squeeze(1),
                ...
                win_length=512,
                hop_length=128,
                window=self.window.to(x0.device),
                center=True,
                normalized=True,
                return_complex=True,
            )
            ...
            pred = solve_adaptive_ode(
                net,
                x0,
                n_steps=n_steps,
                adaptive_band=self.adaptive_band,
    ...
    def l1_loss(self, pred: torch.Tensor, target: torch.Tensor):
        return torch.nn.functional.l1_loss(pred, target, reduction="none").sum()

    def l2_loss(self, pred: torch.Tensor, target: torch.Tensor):
    ...
    def __call__(self, net, x0, x1, **net_conds):
        ...
        return self.loss_func(pred_vf, true_vf) / x0.size(0) , stats
...
if __name__ == "__main__":
    ...
    diffusion = Diffusion("triangle", spectral=spectral, loss_type="l2")
    print(diffusion.loss_func)
    ...
    s = diffusion.generate(
        model,
        torch.randn(2, 1, 32768).to(device),
        n_steps=100,
        **dict(band=band.expand(2, -1)),
    )
    print(s.shape)
