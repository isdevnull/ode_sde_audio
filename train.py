...
class AudioDiffusionTrainer:
    ...
    def setup_loaders(self):
        ...
        self.train_loader = iter(
            hydra.utils.instantiate(
                self.cfg.dataloader.train_dataloader,
                self.train_dataset,
                batch_size=self.cfg.train_batch_size,
                sampler=sampler,
            )
        )
        self.val_loader = hydra.utils.instantiate(
            self.cfg.dataloader.val_dataloader,
            self.val_dataset,
    ...
    def setup_model(self, load_from_checkpoint: bool = False):
        model = hydra.utils.instantiate(
            self.cfg.model.model, spectral=self.cfg.spectral
        )
        if load_from_checkpoint:
            model.load_state_dict(
                torch.load(self.cfg.checkpoint_path, map_location="cpu")
            )
    ...
    def train_epoch(self) -> dict:
        ...
        self.model.train()
        for i in tqdm(
            range(1, self.cfg.n_iters_per_epoch * self.cfg.accumulate_every + 1)
        ):
        ...
        for epoch in range(1, self.cfg.n_epochs + 1):
            ...
            if epoch % self.cfg.visualize_every_epoch == 0:
                ...
                for _, (x, y, b) in enumerate(self.val_loader):
                    ...
                    for idx in range(y_pred.shape[0]):
                        ...
                        log_data.update(
                            {
                                f"test_sample_{idx}/noisy": wandb.Audio(
                                    x[idx].view(-1).cpu().numpy(),
                                    sample_rate=sr,
                                    caption=f"{epoch=}",
                                ),
                                f"test_sample_{idx}/pred": wandb.Audio(
                                    y_pred[idx].view(-1).cpu().numpy(),
                                    sample_rate=sr,
                                    caption=f"{epoch=}",
                                ),
                                f"test_sample_{idx}/ref": wandb.Audio(
                                    y[idx].view(-1).cpu().numpy(),
                                    sample_rate=sr,
                                    caption=f"{epoch=}",
                                ),
                                f"test_sample_{idx}/specs": wandb.Image(
                                    spec_fig, caption=f"{epoch=}"
                                ),
                            }
                        )
            ...
            if epoch % self.cfg.save_model_every_epoch == 0:
                save_dir = os.path.join(
                    self.accelerator.project_configuration.project_dir, "model_ckpt"
                )
                if self.accelerator.is_main_process:
                ...
                self.accelerator.save_model(
                    self.model, os.path.join(save_dir, f"ckpt_{epoch=}")
                )
